			...
			<div class="panel-body">
				...
				<div class="form-horizontal">
					...
					<c:if test="${projeto.status == 'NOVO'}">
						<div class="col-md-4">
							<label style="width: 100%; text-align: right;"> <a
								id="submeter" data-toggle="modal" data-target="#confirm-submit"
								href="#" title="Submeter"
								data-href="<c:url value="/projeto/submeter/${projeto.id}" ></c:url>"
								data-name="${projeto.nome }" class="btn btn-primary btn-sm">
									<i class="fa fa-cloud-upload"></i>
							</a> <c:if
									test="${(not empty projeto.inicio) && (not empty projeto.termino)}">
									<a id="vincular"
								...
								data-target="#confirm-delete" href="#" title="Excluir"
								data-href="<c:url value="/projeto/excluir/${projeto.id}"></c:url>"
								data-name="${projeto.nome }" class="btn btn-danger btn-sm">
					...
					<div class="form-group">
						...
						<div class="col-sm-4 value-label">
							<label><a
								href="<c:url value="/pessoa/detalhes/${projeto.coordenador.id}" ></c:url>">${projeto.coordenador.nome}</a></label>
						</div>
					...
					<div class="form-group">
						...
						<c:if test="${not empty projeto.parecer.dataRealizacao }">
							...
							<div class="col-sm-3 value-label">
								<label> <fmt:formatDate pattern="dd/MM/yyyy HH:mm"
										value="${projeto.parecer.dataRealizacao }" />
								</label>
							</div>
						</c:if>
						...
						<div class="col-sm-10 value-label">
							...
							<c:if test="${not empty projeto.participacoes }">
								<c:forEach items="${projeto.participacoes }" var="participacao">
									<label><a
										href="<c:url value="/pessoa/detalhes/${participacao.participante.id}" >
									</c:url>">${participacao.participante.nome}
									</a>(${participacao.tipo.descricao});</label>
									<br>
								</c:forEach>
							</c:if>
						</div>
					...
					<div class="form-group">
						<label class="col-sm-2 control-label">Observação
							Avaliação:</label>
						<div class="col-sm-10 value-label">
							...
							<c:if test="${not empty projeto.observacaoAvaliacao }">
								<label> ${projeto.observacaoAvaliacao } </label>
							</c:if>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10 value-label">
							...
							<c:if test="${not empty projeto.oficio }">
								<label><a
									href="<c:url value="/documento/${projeto.id }/${projeto.oficio.id }" ></c:url>">${projeto.oficio.nome }</a></label>
								<br>
							</c:if>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-10 value-label">
							...
							<c:if test="${not empty projeto.ata }">
								<label><a
									href="<c:url value="/documento/${projeto.id }/${projeto.ata.id }" ></c:url>">${projeto.ata.nome }</a></label>
								<br>
							</c:if>
						</div>
					</div>
					...
					<c:if test="${projeto.parecer != null}">
						...
						<div class="form-group">
							...
							<div class="col-sm-4 value-label">
								<label><a
									href="<c:url value="/pessoa/detalhes/${projeto.parecer.parecerista.id}" ></c:url>">${projeto.parecer.parecerista.nome}</a></label>
							</div>
							...
							<c:if test="${projeto.status == 'AGUARDANDO_PARECER'}">
								...
								<div class="col-sm-4 value-label">
									<label><fmt:formatDate pattern="dd/MM/yyyy"
											value="${projeto.parecer.prazo }" /></label>
								</div>
							</c:if>
						...
						<div class="form-group">
							...
							<div class="col-sm-10 value-label">
								<a
									href="<c:url value="/documento/${projeto.id }/${projeto.parecer.documento.id }" />">${projeto.parecer.documento.nome }</a>
							</div>
						</div>
					</c:if>
								...
								<div class="panel panel-default">
									<div class="panel-heading">${comentario.autor.nome }
										<span class="date-comment"><fmt:formatDate
												pattern="dd/MM/yyyy" value="${comentario.data }" /> - <fmt:formatDate
												pattern="HH:mm" value="${comentario.data }" /></span>
									</div>
						...
						<div id="div-comentario" class="col-sm-12 form-item">
							<div id="campo-comentario" class="col-sm-12">
								<textarea id="comentario" name="comentario" class="form-control"
									rows="5" placeholder="Comentário" required="required"></textarea>
							</div>
						</div>
						<br>
						<div class="controls">
							<input id="comentar" name="comentar" type="submit"
